...
export interface RoomMember {
  ...
  characterType: CharacterType;
  globalSubLane: number;
  /** Lobby ready check (presence only; absent when read from the DB). */
  ready?: boolean;
}
...
export interface PlayerSnapshot {
  userId: string;
  globalSubLane: number;
  /** Authoritative species when known (auth race). Prefer over lane inference. */
  characterType?: CharacterType;
...
/**
 * Broadcast when any client eats an NPC bot. Identified by global sub-lane
 * (0–8) so every peer hides the same roster filler — spawn slots are
 * deterministic from the shared room roster + world seed.
 */
export interface NpcEatEvent {
  /** Who ate the bot (for logging; receivers key off globalSubLane only). */
  eaterId: string;
  /** Which bot slot was consumed — stable across all clients in the room. */
  globalSubLane: number;
}
