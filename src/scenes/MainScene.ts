...
export class MainScene extends Phaser.Scene {
  ...
  init() {
    // Dynamic server URL for Vercel/Railway deployment
    ...
    this.room.state.players.onAdd((serverPlayer: any, sessionId: string) => {
      ...
      serverPlayer.onChange(() => {
        ...
        if (entity.container.x !== newX) {
          ...
          this.tweens.add({
            targets: entity.container, x: newX, duration: 150, ease: 'Cubic.easeOut',
  ...
  sendMove(direction: number) {
    ...
    if (barrierCrossed > 0 && this.isBarrierSolid(barrierCrossed, localEntity.container.y)) {
      ...
      this.tweens.add({
        targets: localEntity.container, x: this.getMicroPosX(currentMicroPos) + bumpOffset,
        duration: 50, yoyo: true, ease: 'Sine.easeInOut',
  ...
  drawRoad() {
    ...
    for (let i = 0; i <= TOTAL_MICRO; i++) {
      ...
      this.roadGraphics.beginPath(); this.roadGraphics.moveTo(x, startY); this.roadGraphics.lineTo(x, endY); this.roadGraphics.strokePath();
    }
