...
> **⚠ Documentation update (June 2026):** This file is **partially stale**. For accurate current behavior use:
> - [`docs/RACE_MECHANICS.md`](docs/RACE_MECHANICS.md) — gameplay as implemented  
> - [`docs/ABILITIES.md`](docs/ABILITIES.md) — 12 briefcase abilities  
> - [`docs/CODEBASE.md`](docs/CODEBASE.md) — what every file does  
> - [`docs/SCENES_AND_FLOW.md`](docs/SCENES_AND_FLOW.md) — scene navigation  
> - [`docs/DEV_GUIDE.md`](docs/DEV_GUIDE.md) — Ability Lab, tuning, scripts  
> - [`docs/APP_MASTER_SPEC.md`](docs/APP_MASTER_SPEC.md) — tournament product canon  
>
> Known stale sections below: trash **carry** (now auto-jump), Menu-first flow (now Week Hub), some tuning numbers, no ability docs.
...
| **NPC / rival visual offset (solo)** | When player progress lags world scroll, local runner + NPCs shift up (small cap) |
| **NPC / rival visual offset (multiplayer)** | Local runner anchored at `groundY`; others placed from `(their distance − yours)` across the visible road band |
...
| Event | Result |
|-------|--------|
| Player eats NPC | NPC dies (blood), hides; **solo:** respawns after 3.5 s; **multiplayer:** stays dead + `npc:eat` broadcast |
...
- **Race-relative rival placement** — remotes/NPCs positioned from real progress gap vs you; slowdown/trash visibly opens distance
- **NPC eat sync** — `npc:eat` broadcast hides the same bot slot on every client; no respawn in MP
...
| Eat chain | ✓ (NPC synced in MP, remote server) | ✓ |
| Prisoner's Dilemma | ✓ | ✓ |
...
| `checkEating(player)` | Proximity + `canEat()` → death; solo respawns, MP broadcasts |
| `applyRemoteNpcEat(lane)` | Hide bot when a peer's `npc:eat` arrives |
| `killNpc` (private) | Solo → `scheduleRespawn`; MP → permanent + optional broadcast |
| `scheduleRespawn` | Solo only — 3500 ms delay, `LaneNpc.respawn()` |

**Multiplayer NPC sync:** bots are identified by `globalSubLane` (0–8). When any client eats a bot, `RoomSession.sendNpcEat` broadcasts `{ eaterId, globalSubLane }`. Receivers call `applyRemoteNpcEat` — idempotent, no referee (bots are not `room_members`). Dilemma kills against bots use the same path via `onNpcEliminated`.
...
- Places rivals with `rivalProgressGapToScreenOffset(remoteDistance − localDistance, groundY)` — linear mapping from real progress gap to the visible road band (~8% race lead = full ahead spread; tune in `TUNING.multiplayer.rivalVisual`)
- Local player stays at `groundY`; when you slow down, rivals climb — when you surge, they drop back
...
│             │   • Broadcast npc:eat (bot elimination sync)
...
| Broadcast | `npc:eat` | Hide NPC bot by `globalSubLane` on all peers |
